...
import type { PolicyContext, TransitionContext } from './PolicyContext';

export function evaluateTransition(context: TransitionContext): PolicyDecision {
  ...
  const requiredCapability = resolveRequiredCapability(
    context.entityType,
  ...
  if (
    context.currentState === ApprovalStatus.Draft &&
  ...
  const accessDecision = evaluateAccess(
    buildAccessContext(context, organizationId, requiredCapability),
  );
  ...
  if (
    (context.targetState === ApprovalStatus.Approved ||
...
function resolveRequiredCapability(
  entityType: TransitionContext['entityType'],
  currentState: TransitionContext['currentState'],
  targetState: TransitionContext['targetState'],
): Capability | null {
...
function isExpectedSourceState(
  currentState: TransitionContext['currentState'],
  targetState: TransitionContext['targetState'],
): boolean {
...
function hasSubstantiveRationale(value: string | null | undefined): boolean {
