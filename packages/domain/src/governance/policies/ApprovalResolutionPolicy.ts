...
import type { ApprovalContext } from './PolicyContext';

export interface QuorumOutcome {
  readonly resolved: boolean;
  readonly outcome: 'Approved' | 'Rejected' | null;
  readonly participatingDecisionIds: ReadonlyArray<string>;
  readonly consideredDecisionIds: ReadonlyArray<string>;
...
export function evaluateQuorum(context: ApprovalContext): QuorumOutcome {
  ...
  const requestChanges = decisions.filter(
    (decision) => decision.outcome === ApprovalOutcome.REQUEST_CHANGES,
  );
  const participantDecisions = decisions.filter(
    (decision) => decision.outcome !== ApprovalOutcome.REQUEST_CHANGES,
  );
  const participatingDecisionIds = Object.freeze(
    participantDecisions.map((decision) => decision.id),
  );
  ...
  if (context.policy.quorumType === QuorumType.MAJORITY) {
    return resolvedOutcome(
      approvals.length > rejections.length ? 'Approved' : 'Rejected',
      participatingDecisionIds,
      consideredDecisionIds,
      approvals.length,
      rejections.length,
  ...
  if (context.policy.quorumType === QuorumType.SINGLE_REJECT) {
    return resolvedOutcome(
      approvals.length > 0 ? 'Approved' : 'Rejected',
      participatingDecisionIds,
      consideredDecisionIds,
      approvals.length,
      rejections.length,
...
function resolvedOutcome(
  ...
  participatingDecisionIds: ReadonlyArray<string>,
  consideredDecisionIds: ReadonlyArray<string>,
...
function unresolvedOutcome(
  participatingDecisionIds: ReadonlyArray<string> = Object.freeze([]),
  consideredDecisionIds: ReadonlyArray<string> = Object.freeze([]),
  approvalCount = 0,
  rejectionCount = 0,
  abstainedCount = 0,
