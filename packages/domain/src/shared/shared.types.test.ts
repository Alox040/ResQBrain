...
test('allow() returns an allowed PolicyDecision with immutable warnings and context', () => {
  const warnings: PolicyWarning[] = [{ code: 'WARN', message: 'non-blocking' }];
  ...
  assert.throws(() => {
    (decision.warnings as PolicyWarning[]).push({ code: 'MUTATE', message: 'fail' });
  });
  assert.throws(() => {
    (decision.context as Record<string, unknown>).extra = true;
  });
});
...
test('PolicyDecision is discriminated by allowed', () => {
  const allowedDecision: AllowedPolicyDecision = allow();
  const deniedDecision: DeniedPolicyDecision = deny(DenyReason.NO_ACTIVE_ROLE);
  const decisions: readonly PolicyDecision[] = [allowedDecision, deniedDecision];

  assert.equal(decisions[0].allowed, true);
  assert.equal(decisions[1].allowed, false);
});

function _compileTimeDenyRequiresReason(): void {
