...
test('G1-03/INV-A-01/INV-A-02: Algorithm identity is immutable and organization-scoped', () => {
  ...
  assert.throws(() => {
    (algorithm as { organizationId: OrgId }).organizationId = orgB;
  });
  assert.throws(() => {
    (algorithm as { id: AlgorithmId }).id = 'alg-2' as AlgorithmId;
  });
});
...
test('TI-04: Protocol and Guideline reject cross-organization references', () => {
  ...
  assert.throws(() =>
    createGuideline({
      ...
      references: [
        {
          entityId: 'proto-1' as ProtocolId,
          entityType: 'Protocol',
          organizationId: orgB,
        },
      ],
...
test('HC-V-05: content entities reject implicit latest version references', () => {
  assert.throws(() =>
    createMedication({
      ...
      title: 'Medication',
      genericName: 'Generic',
      currentVersionId: 'latest' as VersionId,
      approvalStatus: ApprovalStatus.Draft,
    }),
  );
});
...
function _compileTimeContentPackageHasNoCompositionField(): void {
  const contentPackage = createContentPackage({
