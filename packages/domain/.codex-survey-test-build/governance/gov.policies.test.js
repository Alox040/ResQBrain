...
function createActorFixture(input) {
    const role = createUserRole({
        ...
        scopeTarget: input?.scopeLevel && input.scopeLevel !== ScopeLevel.ORGANIZATION
            ? {
                id: input.scopeTargetId,
                organizationId: input.organizationId ?? orgA,
            }
            : null,
    ...
    const permissions = (input?.permissions ?? [Capability.CONTENT_APPROVE]).map((capability, index) => createPermission({
        id: `perm-${index}`,
...
test('G2-01/G2-02/G2-03/G2-04: evaluateAccess denies missing org, cross-tenant, inactive org, and missing capability', () => {
    ...
    }).denyReason, DenyReason.MISSING_ORGANIZATION_CONTEXT);
    assert.equal(evaluateAccess({
        actor,
        organizationId: orgB,
        ...
        entityId: 'alg-1',
        entityType: EntityType.Algorithm,
    }).denyReason, DenyReason.CROSS_TENANT_ACCESS_DENIED);
    assert.equal(evaluateAccess({
        actor,
        organizationId: orgA,
        ...
        entityId: 'alg-1',
        entityType: EntityType.Algorithm,
    }).denyReason, DenyReason.ORGANIZATION_NOT_ACTIVE);
    assert.equal(evaluateAccess({
        actor,
        organizationId: orgA,
        organization: { id: orgA, status: OrganizationStatus.ACTIVE },
    ...
    }).denyReason, DenyReason.CAPABILITY_NOT_GRANTED);
});
