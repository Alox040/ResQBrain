...
export function evaluateAccess(context) {
    ...
    const grantingRoles = getCapabilityGrantingRoles(context.actor, context.capability, organizationId, {
        entityType: context.entityType,
        targetScope: normalizeScopedTarget(context.targetScope),
    });
    if (grantingRoles.length === 0) {
        const hasAnyActiveRole = context.actor.roles.some((role) => role.userId === context.actor.userId &&
        ...
        const hasSelfAssignment = context.capability === 'role.assign' &&
        ...
        const hasCapabilityIgnoringScope = getCapabilityGrantingRoles(context.actor, context.capability, organizationId, {
            entityType: context.entityType,
        }).length > 0;
    ...
    if (context.capability === 'role.assign' &&
        context.targetUserId != null &&
        context.targetUserId === context.actor.userId) {
        return deny(DenyReason.SELF_ASSIGNMENT_PROHIBITED, {
            organizationId,
...
function normalizeScopedTarget(targetScope) {
    if (targetScope == null) {
        return null;
    }
    return {
        scopeLevel: targetScope.scopeLevel,
        scopeTargetId: 'scopeTargetId' in targetScope ? (targetScope.scopeTargetId ?? null) : null,
    };
}
