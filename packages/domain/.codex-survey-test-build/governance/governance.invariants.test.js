...
test('INV-D-07/08/09/10/11/12: org binding, no admin shortcut, no cross-org grants, no grants without role, released stays immutable', () => {
    ...
    assert.equal(evaluateAccess({
        ...
        entityId: 'alg-1',
        entityType: EntityType.Algorithm,
    }).denyReason, DenyReason.NO_ACTIVE_ROLE);
    assert.equal(evaluateAccess({
        actor,
...
test('governance policies never import survey and entities never import policy/service layers', () => {
    ...
    for (const fileName of policyFiles) {
        const source = readFileSync(join(process.cwd(), 'src/governance/policies', fileName), 'utf8');
    ...
    for (const fileName of entityFiles) {
        const source = readFileSync(join(process.cwd(), 'src/governance/entities', fileName), 'utf8');
        assert.equal(/from ['"]\.\.\/policies\//.test(source) ||
            /lifecycle\/services/.test(source) ||
            /release\/services/.test(source), false, `${fileName} must remain passive foundation code`);
    }
});
