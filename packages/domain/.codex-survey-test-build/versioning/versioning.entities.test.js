...
import { LineageState, ReleaseStatus, ReleaseType, createCompositionEntry, createContentPackageDependencyNote, createContentEntityVersion, createContentPackageVersion, createReleaseVersion, markReleaseVersionSuperseded, withAdditionalContentEntityLineageStates, } from './entities';
const orgA = 'org-A';
const orgB = 'org-B';
...
test('ContentPackageVersion snapshots dependency notes and scope applicability immutably', () => {
    const dependencyNote = createContentPackageDependencyNote({
        ...
        targetEntityId: 'med-1',
        targetVersionId: 'med-ver-1',
        severity: 'HardBlock',
        rationale: 'Medication must be active alongside the algorithm.',
    });
    const version = createContentPackageVersion({
        id: 'pkg-ver-dependency',
        organizationId: orgA,
        packageId,
    ...
    assert.throws(() => {
        version.dependencyNotes.push(dependencyNote);
    });
});
...
test('implementation constraint: versioning foundation uses no common/* imports', () => {
    ...
    for (const fileName of versioningFiles) {
        const source = readFileSync(join(process.cwd(), 'src/versioning', fileName), 'utf8');
