...
export function createStation(input) {
    ...
    if (input.region && input.region.organizationId !== organizationId) {
        throw new DomainError('CROSS_TENANT_SCOPE_REFERENCE', 'Station.regionId must reference a Region within the same organization.', {
            organizationId,
            regionOrganizationId: input.region.organizationId,
        });
    }
    if (input.county && input.county.organizationId !== organizationId) {
        throw new DomainError('CROSS_TENANT_SCOPE_REFERENCE', 'Station.countyId must reference a County within the same organization.', {
            organizationId,
    ...
    if (input.region &&
        ...
        throw new DomainError('DATA_INTEGRITY_VIOLATION', 'Station county and region hierarchy must be consistent.', {
...
function assertOrgId(value) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DomainError('MISSING_ORGANIZATION_CONTEXT', 'Station.organizationId is required.');
    }
    return value;
}
