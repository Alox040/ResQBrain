...
import { assertNoImplicitTenantDerivation, assertOrgActive, assertSameOrg, contentSharingPolicyExists, createOrganization, OrganizationStatus, validateIntraOrgRef, validateOrgIdPresent, } from './index';
const orgA = 'org-A';
const orgB = 'org-B';
...
test('assertOrgActive allows only active organizations for writes', () => {
    ...
    assert.throws(() => assertOrgActive(suspended), (error) => error instanceof DomainError &&
    ...
    assert.throws(() => assertOrgActive(decommissioned), (error) => error instanceof DomainError &&
        error.code === 'ORGANIZATION_NOT_ACTIVE');
});
test('assertSameOrg and validateIntraOrgRef deny cross-tenant references', () => {
    assert.deepEqual(assertSameOrg({ organizationId: orgA }, { organizationId: orgA }), [{ organizationId: orgA }, { organizationId: orgA }]);
    assert.throws(() => validateIntraOrgRef({ organizationId: orgA }, { organizationId: orgB }), (error) => error instanceof TenantIsolationViolation);
});
test('contentSharingPolicyExists remains disabled in phase 0', () => {
    assert.equal(contentSharingPolicyExists(orgA, orgB, 'alg-1'), false);
});
test('implicit tenant derivation from fallback context is rejected', () => {
    assert.throws(() => assertNoImplicitTenantDerivation(undefined, {
        currentOrg: orgA,
