...
exports.createContentEntityVersion = createContentEntityVersion;
exports.withAdditionalContentEntityLineageStates = withAdditionalContentEntityLineageStates;
const errors_1 = require("../../shared/errors");
const LineageState_1 = require("./LineageState");
const EntityType_1 = require("./EntityType");
function createContentEntityVersion(input) {
...
function withAdditionalContentEntityLineageStates(version, additions) {
    return Object.freeze({
        ...version,
...
function validateContentEntityPredecessor(input, predecessor) {
    ...
    if (predecessor.entityId !== input.entityId) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'predecessorVersionId must reference the same entity lineage.');
    }
    if (predecessor.entityType !== input.entityType) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'predecessorVersionId must reference the same entity type.');
    }
...
function assertEntityType(value) {
    if (!EntityType_1.CONTENT_ENTITY_TYPES.includes(value)) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'entityType must be a content entity type.', { entityType: value });
    }
    return value;
}
...
function deepFreezeSnapshot(snapshot) {
    if (snapshot === null || Array.isArray(snapshot) || typeof snapshot !== 'object') {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'snapshot must be a plain object.');
    }
