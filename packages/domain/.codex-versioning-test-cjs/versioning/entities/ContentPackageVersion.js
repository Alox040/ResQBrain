...
exports.createContentPackageVersion = createContentPackageVersion;
exports.withAdditionalContentPackageLineageStates = withAdditionalContentPackageLineageStates;
exports.freezeTargetScope = freezeTargetScope;
const errors_1 = require("../../shared/errors");
const entities_1 = require("../../tenant/entities");
const CompositionEntry_1 = require("./CompositionEntry");
const LineageState_1 = require("./LineageState");
function createContentPackageVersion(input) {
    ...
    const composition = Object.freeze(input.composition.map((entry) => (0, CompositionEntry_1.createCompositionEntry)(entry)));
    ...
    return Object.freeze({
        ...
        lineageState: (0, LineageState_1.createLineageStateSet)([LineageState_1.LineageState.ACTIVE]),
        createdAt,
...
function withAdditionalContentPackageLineageStates(version, additions) {
    return Object.freeze({
        ...version,
        lineageState: (0, LineageState_1.appendLineageStates)(version.lineageState, additions),
    });
}
function validateContentPackagePredecessor(input, predecessor) {
    if (predecessor.organizationId !== input.organizationId) {
        throw new errors_1.DomainError('CROSS_TENANT_ACCESS_DENIED', 'predecessorVersionId must stay within the same organization.');
    }
    if (predecessor.packageId !== input.packageId) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'predecessorVersionId must reference the same package lineage.');
    }
    if (predecessor.hasSuccessor === true) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'Branching is prohibited in Phase 0.');
    }
}
function freezeTargetScope(input) {
    if (input == null || typeof input !== 'object') {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'targetScope is required.');
    }
    if (input.scopeLevel === entities_1.ScopeLevel.ORGANIZATION) {
        if (input.scopeTargetId != null) {
            throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'Organization targetScope must not define scopeTargetId.');
        }
        return Object.freeze({
...
function assertRequiredChangeReason(value) {
    ...
    if (normalized == null) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'changeReason is required from version 2 onward.');
    }
    return normalized;
}
...
function normalizeOptionalText(value) {
    ...
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new errors_1.DomainError('DATA_INTEGRITY_VIOLATION', 'Optional text fields must be non-empty when provided.');
    }
    return value.trim();
}
