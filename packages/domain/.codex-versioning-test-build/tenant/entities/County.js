...
export function createCounty(input) {
    const organizationId = assertOrgId(input.organization.id);
    if (input.region && input.region.organizationId !== organizationId) {
        throw new DomainError('CROSS_TENANT_SCOPE_REFERENCE', 'County.regionId must reference a Region within the same organization.', {
            organizationId,
...
function assertOrgId(value) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new DomainError('MISSING_ORGANIZATION_CONTEXT', 'County.organizationId is required.');
    }
    return value;
}
