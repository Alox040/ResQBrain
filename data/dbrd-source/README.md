...
| Ordner | Zweck |
|--------|--------|
| `raw/` | **Unveränderte Quelldaten** — Unterordner `medications/`, `algorithms/`; Liefermetadaten in `raw/meta/sources.json`. Keine inhaltliche Bereinigung hier. Importregeln: `docs/context/dbrd-import-rules.md`. |
...
2. **Keine direkte Bearbeitung** der produktiven App-JSONs unter `data/lookup-seed/` ohne dokumentierten Transform (Skript, Mapping, Review). Änderungen an Quellen oder Normalisierung laufen über `raw/` → `normalized/` → Generierung/Update des Seeds. JSON-Rohdaten → normalisiert: `pnpm dbrd:normalize` (siehe `scripts/dbrd/README.md`).
...
- Rohdaten-Import und Quellentrennung: `docs/context/dbrd-import-rules.md`
